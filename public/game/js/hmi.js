...
Hmi.prototype.renderStatus = function(board, actionInfo) {
  ...
  if (board.previous.type != 'none') {
    ...
    status +=
      ...
      (board.ply >> 1) +
      '. ' +
      (board.previous.type == 'pass'
        ? previousActionPlayer + ' passed.'
        : ('Black' == previousActionPlayer
            ? pos + '&#8230;'
            : '&#8230;' + pos) +
          ' by ' +
          previousActionPlayer +
          ' flipping ' +
          board.previous.flip.length +
          ' checkers.')
  }
  if (actionInfo) {
...
Hmi.prototype.update = function(board, actionInfo) {
  ...
  for (var y = 0; y < 8; ++y) {
    for (var x = 0; x < 8; ++x) {
      $('#field' + String.fromCharCode(97 + x) + (y + 1)).html(
        this.checker[board.square[x][y]]
      )
    }
  }
...
Hmi.prototype.prepareHumanMove = function(board) {
  ...
  for (var i = 0; i < board.actions.length; ++i) {
    var field =
      '#field' +
      String.fromCharCode(97 + board.actions[i].x) +
      (board.actions[i].y + 1)
...
Hmi.prototype.deactivateClicks = function() {
  for (var i = 0; i < this.board.actions.length; ++i) {
    var field =
      '#field' +
      String.fromCharCode(97 + this.board.actions[i].x) +
...
Hmi.prototype.clickHandler = function(event) {
  ...
  var move = {
    type: 'set',
    x: event.currentTarget.id[5].charCodeAt(0) - 97,
